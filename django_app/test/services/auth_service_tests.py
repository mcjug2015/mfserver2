...
class RefererTokenSessionHelperTests(TestCase):
    ...
    def test_get_referer(self):
        ...
        helper = auth_service.RefererTokenSessionHelper(None, None, "https://www.test.com")
        self.assertEqual("https://www.test.com", helper.get_referer())

    def test_get_token_session(self):
        ''' test the get token session method '''
        helper = auth_service.RefererTokenSessionHelper("user", "pass", "https://www.test.com")
        ...
        when(auth_service.requests).get("https://www.test.com/mfserver2/welcome/",
                                        verify=False).thenReturn(get_response)
        post_response = mock()
